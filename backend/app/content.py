"""Word-banks for the AI-slop profile factory (Python port of js/generator.js).

Kept as the server-side source of truth for *served* profiles. Client-side
generation in js/generator.js remains the offline fallback; the two can drift
in wording without breaking anything, since a profile only needs a stable
`seed` string for the client to render its avatar.
"""
...
AI_ARTIFACTS = [
    "6 fingers, extremely confident", "background melts if you stare", "teeth: yes",
    "generated at temperature 1.4", "hands rendered by committee",
    "eyes track you across the room", "this jawline is load-bearing",
]
